...
workspace_sync() {
    ...
    ssh $WORKSPACE_SSH_OPTS $ssh_user_expr $WORKSPACE_REMOTE_HOST "mkdir -p $WORKSPACE_REMOTE_PATH/$rel_path"

    # sync the files to remote server.
    # example of WORKSPACE_REMOTE_SYNC_OPTS="--delete --exclude target --exclude .build"
    rsync -e "ssh $WORKSPACE_SSH_OPTS $ssh_user_expr" ${WORKSPACE_RSYNC_OPTS:-"-crptvzL"} $PWD/ $WORKSPACE_REMOTE_HOST:$WORKSPACE_REMOTE_PATH/$rel_path/
}
